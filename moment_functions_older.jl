...
function calc_production_resids!(it0,R,data,pars1)
    ...
    # this function call is assumed to return a coefficient lΦm where $X_{it} = τ_{m,it} / exp(lΦm)
    lΦm,lΦf,lΦg,log_price_97 = calc_Φ_m_older(pars1,data,it0)
    Ψ0 = 0
    coeff_X = pars1.δ[1]*pars1.δ[2]^4
    for t in 1:4
        lΦm,lΦf,lΦg,log_price_index = calc_Φ_m_older(pars1,data,it0+t)
        #lϕm,lϕf,lϕc,log_price_index,Φg = log_input_ratios_older(pars1,data,it0+t)
